...
@section('content')
    @foreach ($absent->absentEmployeeData as $item)
            ...
            @if ($item->status == 'attendence')
                <div class="bg-main-color flex flex-row gap-5 items-center p-2 rounded-xl pl-3">
                    ...
                    <div class="w-8 h-8 rounded-full overflow-hidden">
                        @if (empty($item->previllage->user->image))
                        ...
                        @else
                            <img class="w-full h-full rounded-full object-center object-cover"
                                src="{{ url('storage/profile/', $item->previllage->user->image) }}" alt="Profile Image">
                        @endif
                    </div>
    ...
    @if ($absent->absentEmployeeNote->count() !== 0)
        <p
            ...
            {{ $absent->absentEmployeeNote->first()->note }}
        </p>
    @endif
@endsection
