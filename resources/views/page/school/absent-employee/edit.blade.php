...
@section('content')
    <form action="{{ route('previlage.absentEmployee.update', [$school->slug, $absent->id]) }}"
        method="post">
        @csrf
        @method('POST')
        @foreach ($teacher as $item)
            <div class="px-4 py-1 md:px-5 md:py-1">
                <div class="bg-white flex flex-row gap-5 items-center p-2 rounded-xl pl-3">
                    ...
                    <input type="text" name="employee[]" value="{{ $item->id }}" hidden>
        ...
        <div class="flex flex-col px-4 py-1 md:px-5 md:py-3">
            <label for="note" class="mb-2 font-Nunito text-white font-bold bg-blue-500 size-fit py-1 px-3">Note</label>
            <textarea name="note" id="note" cols="3" rows="5"></textarea>
            <button type="submit"
